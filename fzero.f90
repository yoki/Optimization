...
contains
	...
	real(kind=d) :: brent0
	real(kind=d), intent(IN) :: x1, x2, tol
	real(kind=d), external :: fun
	integer, intent(IN), optional :: detail
	integer :: i, exitflag, disp
	real(kind=d) :: a, b, c, diff,e, fa, fb, fc, p, q, r, s, tol1,xm,tmp 
	real(kind=d), parameter :: EPS = epsilon(a)
	...
	integer, parameter :: d = selected_real_kind(p=13,r=200)
	real(kind=d) :: brent
	real(kind=d), intent(IN) :: x0, tol
	real(kind=d), external :: fun
	real(kind=d), intent(IN), optional :: LowerBound, UpperBound
	integer, intent(IN), optional :: detail
	
	real(kind=d) :: a , b , olda, oldb, fa, fb
	real(kind=d), parameter :: sqrt2 = sqrt(2.0_d)! change in dx
	integer, parameter :: maxiter = 40
	real(kind=d) :: dx  ! change in bracket
...
! 	out = brent(fun =fun1, x0 = 4.0_d, tol = 0.0000001_d, & 
! 		LowerBound= 0.0_d, detail = 1)

	! test function 2
! 	write(*,*) 'Function 2: answer =  -3 '
! 	out = brent(fun2, 3.0_d,0.00001_d, detail = 1)

	write(*,*) 'Function 3: answer =  8.10451, 10, 11.8955' 
	out = brent(fun3, 0.0_d, 0.000001_d, detail = 34)
